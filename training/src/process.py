...
def get_features(target: str, features: list, data: pd.DataFrame):
    ...
    # for pasty: https://patsy.readthedocs.io/en/latest/quickstart.html
    # -1 means: we dont use Intercept column
    y, X = dmatrices(
...
def rename_columns(X: pd.DataFrame):
    X.columns = X.columns.str.replace(r"\[", r"_", regex=True).str.replace(
        r"\]", "", regex=True
    )
    return X
